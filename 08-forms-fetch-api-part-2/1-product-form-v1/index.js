...
export default class ProductForm {
  ...
  onImageChoose = () => {
    ...
    fileInput.addEventListener('change', async (event) => {
      ...
      imageListContainer.firstElementChild.insertAdjacentHTML('beforeend',
                         this.renderImageElement(escapeHtml(image.name), escapeHtml(result.data.link)));        
      } catch (error) {
          throw new Error(`Unable to upload picture to ${IMGUR_URL}. Details: ${error}`);
      }
    });
  ...
  constructor (productId) {
    ...
    this.url = {product : new URL(`/api/rest/products`, BACKEND_URL),
                categories : new URL(`/api/rest/categories`, BACKEND_URL)};
  }
  ...
  getImageElements() {
    return this.product.images.map(image => this.renderImageElement(image.source, image.url)).join('');
  }
  ...
  async save() {
    ...
    try {
      ...
      this.element.dispatchEvent(new Event(this.productId? 'product-updated' : 'product-saved'));
    } catch(error) {
